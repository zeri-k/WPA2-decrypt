...
def generate_ptk(pmk: bytes, anonce: bytes, snonce: bytes, AP_Addr: bytes, STA_Addr: bytes):
    ...
    kek = bytes.fromhex(ptk[32:64])
    tk = bytes.fromhex(ptk[64:96])
    mic_tx = bytes.fromhex(ptk[96:112])
    mic_rx = bytes.fromhex(ptk[112:])
    return ptk, kck, kek, tk, mic_tx, mic_rx

def eapol_key(packet):
...
def generate_gtk(kek: bytes, key_wrap_data: bytes):
...
def get_ccmp_iv(packet) -> Tuple[bytes, bytes]:
...
def main():
    ...
    for packet in packet_list:
        ...
        if radioTap.haslayer(EAPOL) and radioTap[EAPOL].type == 3:  # EAPOL_KEY
            eapol_message = eapol_key(radioTap)
    ...
    if Dot11CCMP in radioTap:
        ccmp_iv, encrypted_data = get_ccmp_iv(radioTap)
